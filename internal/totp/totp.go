...
// Atomic Site's admin two-factor authentication, using only the
// standard library plus golang.org/x/crypto/bcrypt. Vendored from the
// Dockyard implementation (kept in lockstep so the two products
// share enrollment + verify semantics).
package totp
