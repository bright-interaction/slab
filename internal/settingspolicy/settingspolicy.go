// Package settingspolicy is the single source of truth for site-settings
// write policy: which categories an agent may write, and how each
// category.key value is validated.
//
// It exists because the policy used to live in three hand-synced copies
// (internal/handlers/settings_validate.go, internal/mcp/helpers.go, and
// the writable-set in internal/agent/settings_catalog.go) that drifted
// repeatedly: the MCP copy was missing ten analytics validators and the
// search category, so the same write was rejected on one surface and
// accepted on another. Both surfaces now delegate here; add new settings
// policy HERE only.
...
// agentWritableCategories enumerates settings categories an agent is
// allowed to write. Anything not in this set is read-only via the agent
// API + MCP (the human admin still controls them in the UI).
//
...
//   - payments: Mollie keys are bearer-equivalent secrets
...
// AgentWritableCategory reports whether agents may write settings in the
// category.
func AgentWritableCategory(category string) bool {
	return agentWritableCategories[category]
}

// AgentWritableCategories returns the writable category names, sorted.
// Used to build the settings catalog summary and the MCP tool schema
// enum so every surface advertises the same policy.
func AgentWritableCategories() []string {
	out := make([]string, 0, len(agentWritableCategories))
	for c := range agentWritableCategories {
		out = append(out, c)
	}
	sort.Strings(out)
	return out
...
// the value is acceptable, or a user-facing error when it isn't.
//
// Empty values are always accepted (they clear the row). Validation only
// fires on non-empty input: typing nothing into a field is the
// documented way to revert to a default, and the settings system has
// always treated "" as "unset".
//
// Validators mirror the catalog in internal/agent/settings_catalog.go.
// When you add a setting there, add the matching validator here so
// admin UI, agent REST, and MCP writes all gate identically.
func ValidateSetting(category, key, value string) error {
	...
	case "security":
		return validateSecurity(key, value)
	case "design":
		return validateDesign(key, value)
	}
	// Unknown categories pass through; future-compat for whatever the
	// next phase introduces. Write ACCESS is guarded separately by
	// AgentWritableCategory on the agent surfaces.
	return nil
}
...
func validateSecurity(key, value string) error {
