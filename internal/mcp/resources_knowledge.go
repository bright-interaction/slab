...
// slab://knowledge/index; each doc is reachable at
// slab://knowledge/<slug> with mimeType text/markdown.
//
...
func (s *Server) registerKnowledgeResources() {
	...
	register(Resource{
		URI:         "slab://knowledge/index",
		Name:        "Knowledge catalog",
		Description: "Catalog of every curriculum doc the agent can read: stack mastery (Astro, TypeScript, the CSS-variable system, blocks, i18n, security, personalization, cookieproof) and UX mastery (typography, color, spacing, motion, accessibility, performance, forms, nav, dark mode, premium-design principles). Bodies are stripped here; fetch slab://knowledge/<slug> for the full text.",
		MimeType:    "application/json",
		Reader: func(ctx context.Context, agent *authmw.AgentIdentity) (string, error) {
			return mustJSON(map[string]any{
				"docs": knowledge.Catalog(),
				"hint": "Read slab://knowledge/<slug> for full body. Use the master_the_stack prompt to walk the curriculum in reading order.",
			}), nil
		},
	})
	...
	for _, slug := range knowledge.Slugs() {
		...
		register(Resource{
			URI:         fmt.Sprintf("slab://knowledge/%s", slug),
			Name:        doc.Title,
