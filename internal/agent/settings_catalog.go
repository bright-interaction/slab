...
func buildSettingsCatalog(siteID string, settingsMap map[string]string) SettingsCatalogInfo {
	...
	// Policy lives in internal/settingspolicy (shared with the REST +
	// MCP write gates, so the catalog can never advertise a different
	// policy than the gates enforce).
	...
	for cat := range allCats {
		if settingspolicy.AgentWritableCategory(cat) {
