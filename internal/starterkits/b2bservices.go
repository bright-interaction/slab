...
func (k *b2bServicesKit) applyKnowledgebase(ctx context.Context, q *store.Queries, siteID string) error {
	entries := []kbDef{
		...
		{Category: "conversion", Title: "Lead magnet pattern", Content: "On every page, offer a tangible artefact in exchange for an email: industry report, audit checklist, ROI calculator. Slab's form blocks integrate with BrightCRM webhook so submissions appear in the CRM with attribution."},
	}
