...
test.describe('Store orders admin', () => {
	...
	test('checkout creates a pending order that lands in admin', async ({
		...
		const resp = await request.post(u(`/sites/${site.id}/checkout`), {
			data: {
	...
	test('state machine rejects illegal pending->fulfilled transition', async ({ request }) => {
		const created = await request.post(u(`/sites/${site.id}/checkout`), {
			data: {
		...
		const r = await request.post(u(`/sites/${site.id}/orders/${orderID}/status`), {
