...
test.describe('Revisions API + UI', () => {
	...
	test('empty entity returns empty revisions list', async ({ adminApi }) => {
		...
		const create = await adminApi.post(u(`/sites/${site.id}/pages`), {
			data: { title: 'Revisions test', slug: 'rev-test', layout: 'default' }
		});
		expect(create.status()).toBe(201);
		const page = await create.json();

		const list = await adminApi.get(u(`/sites/${site.id}/revisions/page/${page.id}`));
	...
	test('page edit creates a revision and restore reverts the page', async ({ adminApi }) => {
		...
		const created = await adminApi.post(u(`/sites/${site.id}/pages`), {
			data: { title: 'Original', slug: 'restore-flow', layout: 'default' }
		});
		expect(created.status()).toBe(201);
		const page = await created.json();

		const patch1 = await adminApi.patch(u(`/sites/${site.id}/pages/${page.id}`), {
			data: { title: 'Edited Once' }
		});
		expect(patch1.status()).toBe(200);

		// Revision row exists capturing the original (pre-edit) state.
		const list1 = await adminApi.get(u(`/sites/${site.id}/revisions/page/${page.id}`));
		...
		const patch2 = await adminApi.patch(u(`/sites/${site.id}/pages/${page.id}`), {
			data: { title: 'Edited Twice' }
		});
		expect(patch2.status()).toBe(200);

		const list2 = await adminApi.get(u(`/sites/${site.id}/revisions/page/${page.id}`));
		...
		const restore = await adminApi.post(
			u(`/sites/${site.id}/revisions/page/${page.id}/1/restore`),
			{ data: {} }
		);
		...
		const list3 = await adminApi.get(u(`/sites/${site.id}/revisions/page/${page.id}`));
	...
	test('invalid entity_type returns 400', async ({ adminApi }) => {
		const resp = await adminApi.get(
			u(`/sites/${site.id}/revisions/site_branding/anything`)
		);
