<script lang="ts">
	...
	async function load() {
		...
		try {
			...
			drafts = Object.fromEntries(
				blocks.map((b) => [
					b.id,
					{ name: b.name, data_json: b.data_json || '{}', dirty: false }
				])
			);
		} catch (err) {
	...
	async function saveOne(id: string) {
		...
		try {
			...
			drafts = {
				...drafts,
				[id]: { name: updated.name, data_json: updated.data_json, dirty: false }
			};
			toast.success('Saved');
		} catch (err) {
						...
						<div class="rounded-xl border border-border-light bg-bg-surface">
							<div class="flex items-center gap-3 px-4 py-3">
								...
								>
									...
									<span class="truncate text-[13px] text-text-primary">{block.name}</span>
									{#if draft?.dirty}
