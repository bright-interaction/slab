				...
				apps: [
					{
						...
						status: 'shipped',
						body:
							'Wildcard record *.atomicsite.example.com -> 203.0.113.10 covers any built-site slug. Wildcard TLS cert covers the same. No per-site DNS or cert work needed for slugs on the default domain. Custom domains route via Dockyard or your own DNS provider.',
					...
					},
					{
						...
						status: 'shipped',
						body:
							'Built sites land at /srv/atomicsite/<slug>/dist via a Caddy bind mount. The wildcard route http://*.atomicsite.example.com serves any slug straight from disk. Local-kind deploy targets land here directly.',
						setup: 'Automatic. Operated by the Dockyard control plane.',
						linkLabel: null,
						linkHref: null
					},
					{
						name: 'the CI system CI deploy pipeline',
						status: 'shipped',
						body:
						...
						setup: 'Automatic. CI status visible at code.brightinteraction.com.',
						linkLabel: null,
						linkHref: null
					},
					{
